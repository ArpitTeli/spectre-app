...
### Terrain intelligence, model selection, and the synthetic data generation pipeline for Spector's Tasking Layer

This is a companion to the main Spector build spec — it covers everything needed to actually train the Tasking Layer's model in detail, developed after realizing several of these pieces are hard dependencies of each other, not independent nice-to-haves. Read alongside the main spec's Sections 3-5, which this expands and in some places corrects.
...
### 2.3.1 Teacher/strategic-tier output schema (confirmed against a real pipeline run)
The strategic tier — both the live LLM and the training-data teacher model — outputs intent and constraints, never a dense route (Section 2.2). Confirmed working format, validated against an actual test run of the full pipeline:
```json
{
  "unit_id": "friendly_1",
  "intent": "attack",
...
```
`avoid_zones` and `engage_zones` are separate lists, not one combined field — an order entering a coordinate inside an `avoid_zone` is a violation; the same geometry inside an `engage_zone` is expected, and the only thing to check there is whether the ordered unit's type is a poor match for that threat (Section 4.4). The four-part `reasoning` structure (situation assessment, tactical choice, tradeoffs considered, consequence of not doing this) is what a judge model (Section 4.5) actually evaluates — richer than a single free-text justification, and it's what let a real test run score an order 8-9/10 on both tactical coherence and reasoning quality rather than just "sounds plausible."
...
One row **per order**, not per example — a two-order example (as seen in the real test run) must let each order be accepted/rejected independently, so one order's failure never discards a good sibling order:
```
...
```
`example_id` groups orders that came from the same scenario, for when you want to export whole examples together; `final_status` still lives per-order. Every stage reads rows in a given status, does its work, writes results, advances the status. A crash mid-run means re-running the pipeline, not losing progress — it picks up wherever each row left off.

**Bug found in the first real end-to-end pipeline run, fixed here:** the original per-example design meant a judge API failure on one order (empty/malformed response — infrastructure noise, not an actual reviewed verdict) caused the whole example, including a sibling order that scored well on both judges, to be marked `rejected`. Per-order rows fix this structurally.

### 4.4 Geometric filter (Stage 4)
Checks two different things, because `avoid_zones` and `engage_zones` (Section 2.3.1 / main spec Section 3) mean opposite things geometrically:
- **`avoid_zones`** — hard no-entry. Any anchor or target point inside one is a violation, full stop.
- **`engage_zones`** — entry is intentional (the unit is closing on that threat to fight it). The check here isn't "did it enter," it's "is the unit type sent one this threat is dangerous to" (`vulnerable_unit_types` on the contact, main spec Section 3) — e.g. a helicopter ordered to solo-engage a SAM site, or a tank frontally engaging an anti-tank position.

The original version of this filter (before the avoid/engage split existed) treated every `known_contacts` radius as a hard no-entry zone and only checked whether the reasoning text *claimed* avoidance — this is the exact failure mode found in the DeepSeek sample (reasoning claims "outside the SAM radius" while target coordinates say otherwise) and also why the real test run's `friendly_1` order (deliberately closing on an MRAP to engage it) would have been wrongly flagged under the old logic. Runs before the judge stage since it's cheap/deterministic and should filter obvious errors before spending judge-model calls on them.
...
**Bug found in the real test run, fixed here:** a judge response that fails to parse as valid JSON (empty response, malformed output — an infrastructure/API failure) was being treated identically to "the judge reviewed this and rejected it." These are not the same thing and must not resolve the same way. A parse failure must trigger a retry (with backoff, tracked via `judge_1_retry_count`/`judge_2_retry_count` in Section 4.3) before falling back to `flagged` for human review — never an automatic `rejected` on the first failure.

### 4.6 Resolution logic (Stage 6), per order
- Both judges agree the order is sound -> `accepted`
- Judges disagree (both successfully returned a verdict) -> `flagged` for human review, NOT auto-rejected — disagreement often signals genuine tactical ambiguity (more than one reasonable answer exists), not necessarily an error
- Failed the geometric filter -> `rejected` outright, does not reach the judges
- A judge call fails to parse -> retry per Section 4.5; only after retries are exhausted does it become `flagged`, never an automatic `rejected`

### 4.7 Human review (Stage 7)
Manually review a random sample of `accepted` rows (roughly 100 of a few thousand) plus every `flagged` row before committing to the export. The geometric filter and judges catch what they're each designed for; this is the final layer for anything neither catches (a plan that's geometrically consistent and logically coherent but still tactically weak, for instance).
...
dataset = load_dataset("json", data_files="spector_training_set.jsonl", split="train")
...
```python
model.save_pretrained("./spector-tasking-adapter")   # 50-200MB adapter, not the full model
model.save_pretrained_gguf("./spector-tasking-gguf", quantization_method = "q4_k_m")  # ready for llama.cpp
```
