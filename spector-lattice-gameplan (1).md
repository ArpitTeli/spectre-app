...
Spector reproduces that loop, scoped to one simulation environment (Arma 3) and infantry/vehicle units, with room to expand later. The loop, end to end:
...
`vulnerable_unit_types` on each contact lists which friendly unit types that threat is genuinely dangerous to — a SAM site threatens helicopters, not infantry; an anti-tank site threatens tanks/IFVs, not infantry. This distinguishes **avoid zones** (a threat's radius that no unit should enter — a minefield, a threat outside this squad's mission) from **engage zones** (the same kind of radius, but around something a unit is deliberately closing on to fight — entering it is the point, not a violation). The check that matters for an engage zone isn't "did you enter it," it's "is the unit entering it one this threat is dangerous to" — see Section 4.3's unit-threat suitability check.
...
- **Unit-threat suitability check:** for any order whose route/target enters a threat's *engage zone* (Section 3 — intentional entry, not a violation like an avoid zone), verify the ordered unit's type is not listed in that contact's `vulnerable_unit_types`. A helicopter ordered to solo-engage a SAM site, or a tank ordered to frontally engage an anti-tank position, is the mismatch this catches — entering the zone was intended, but the wrong unit type was sent to do it.
...
- Chinese-origin models anywhere near this pipeline being reused later in the actual Straegis product - fine as a cheap synthetic-data-generation option for Spector specifically if desired, not fine to bleed into anything defense-proper
